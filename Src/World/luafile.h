...
namespace World {
	...
	public:
		...
		template<class ParameterType, class ErrorHandler>
		bool CallFunction(const IdentifierPath path, const ParameterType arguments[], const ErrorHandler luaErrorHandler) {
			const int stackSize = lua_gettop(m_pLua);
			if (moveToLevel(path, false) == -1) {
				return false;
			}

			const int arraySize = sizeof(arguments) / sizeof(arguments[0]) + 1;
			for (int i = 0; i < arraySize; i++) {
				push<ParameterType>(arguments[i]);
			}

			const bool result = lua_pcall(m_pLua, arraySize, 0, 0) == 0;
			if (!result) {
